...
class PointLineErrorTerm2D{
public:
    PointLineErrorTerm2D(const double a0, const double a1,
    ...
    bool operator()(const T* const x, const T* const y,
        ...
        Eigen::Matrix<T, 2, 1> a_casted = a.cast<T>();
        Eigen::Matrix<T, 2, 1> n_casted = n.cast<T>();
        Eigen::Matrix<T, 2, 1> dist_vec = (a_casted-p) - ( (a_casted-p).dot(n_casted) ) * n_casted;

        if(dist_vec(0,0)==T(0) && dist_vec(1,0)==T(0)){
            dist_vec(0,0) = T(1e-6);
        }
    ...
    static ceres::CostFunction* Create2D(const double a0, const double a1,
                                         const double n0, const double n1){
        return (new ceres::AutoDiffCostFunction<PointLineErrorTerm2D, 1, 1, 1>( new PointLineErrorTerm2D(a0, a1, n0, n1) ) );
    }
