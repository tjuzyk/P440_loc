               ...
               rcmMsg_EchoedRangeInfo *echoedInfo)
{
    ...
    memset(rangeInfo, 0, sizeof(*rangeInfo)); //内存空间初始化
    ...
    while ((numBytes = rcmIfGetPacket(&infoMsgs, sizeof(infoMsgs))) > 0)
    {
        ...
        switch(ntohs(infoMsgs.rangeInfo.msgType))   //判断msgType
        {
            case RCM_FULL_RANGE_INFO:
                // copy message to caller's struct
                memcpy(rangeInfo, &infoMsgs.rangeInfo, sizeof(*rangeInfo));  //将FULL_RANGE_INFO信息复制进rangeInfo
                // handle byte ordering
