    ...
    m_nh(nh), m_retry_rate(5)
{
    ...
    // init ros topic 发布ros话题
    m_full_range_info_pub = m_nh.advertise<common_msgs::UWB_FullRangeInfo>("full_range_info", 10);  //full_range_info话题为slap_pp节点订阅的话题，后续执行EKF等
...
void uavos::TimeDomainInterface::loadParameters(){
    ...
    //m_nh.param<std::string>("ip_address", m_options.ip_address, std::string("192.168.1.100"));    采用USB连接将其注释掉
    ...
    m_nh.param<std::string>("network_type", m_options.network_type, "ALOHA");   //默认为TDMA
    ...
    m_nh.param<int>("max_request_data_size", m_options.max_request_data_size, 100);  //默认为10
...
int uavos::TimeDomainInterface::preInit(){
    ...
    // Put radio in Idle
    if (rcmSleepModeSet(RCRM_SLEEP_MODE_IDLE) != 0)   // 设置Sleep Mode
    {
    ...
    if (rcmOpModeSet(RCRM_OPMODE_RCM) != 0)           // 设置 OpMode 为 RCM，而非RN
    {
...
int uavos::TimeDomainInterface::RCMInit(){
    // retrieve RCM config
    if (rcmConfigGet(&m_rcmConfig) != 0)     //得到初始的RCM数据
    {
    ...
    //
    m_rcmInitConfig = m_rcmConfig;	// Save initial configuration to restore when done  初始的RCM配置信息
...
int uavos::TimeDomainInterface::TDMASlotmapInit(){
    ...
    if(m_rnInitTdmaSlotMap!=slotmap_request){
        ...
        if (rnTdmaSlotMapSet(&slotmap_request) != 0)   //更新SlotMap 重点看这个函数
        {
...
int uavos::TimeDomainInterface::setupUWB(){
    loopWrapper<int>(&uavos::TimeDomainInterface::preInit, this);  //打印status Info
    loopWrapper<int>(&uavos::TimeDomainInterface::RCMInit, this);  //获得Initial RCM config , 根据launch文件参数判断是否需要修改RCM config ，如果是则修改并打印修改后的RCM config
    loopWrapper<int>(&uavos::TimeDomainInterface::RNInit, this);   //获得Initial RN config ， 根据launch文件参数判断是否需要修改RN config ，如果是则修改并打印修改后的RN config
    if(std::string("TDMA")==m_options.network_type){
        loopWrapper<int>(&uavos::TimeDomainInterface::TDMAInit, this);  //获得Initial TDMA config ， 根据~~~~~~~同上
        loopWrapper<int>(&uavos::TimeDomainInterface::TDMASlotmapInit, this); //获得Initial TDMASlotmap并打印，根据txt文件判断是否需要修改Slotmap，如果是则修改并打印修改后的结果
    } else {
        loopWrapper<int>(&uavos::TimeDomainInterface::ALOHAInit, this);  // ALOHA初始化没看
    }

    if(false==m_is_rewrite_RN_config){
        // don't rewrite RN config, i.e., RCM mode after reboot
        loopWrapper<bool>(&uavos::TimeDomainInterface::activateRadio, this); //激活模块
    } else {
...
bool uavos::TimeDomainInterface::activateRadio(){
    ...
    // operation mode at RCM after reboot. 不更改RN config而直接设置opmode将导致重启后opmode为RCM模式
    //(即在软件里点了networking按钮，开始组网测距，但是变黄的set按钮没点。这时如果再次上电，模块将不会自动进行组网测据而是在RCM模式下，需要手动点击networking按钮开始测距)
...
bool uavos::TimeDomainInterface::readAMessage(){
    ...
    switch(rcmInfoGetFull(&rangeInfo, &ndbInfo, &dataInfo, &echoedInfo))   // 判断接受到的消息类型（rangeInfo、ndbInfo、dataInfo）并储存
    {
        case RANGEINFO:
        {
...
//                        rangeInfo.filteredRangeVel, rangeInfo.filteredRangeVelErrEst);
            ...
            if(StatusSuccess==rangeInfo.rangeStatus){
                m_full_range_info_pub.publish(rcmRange2ros(m_options.node_id, rangeInfo));  // 发布ros消息
            } else {
        ...
        case FULLNDB:
        {
            ...
            m_NDB_pub.publish(rnNDB2ros(m_options.node_id, ndbInfo));  //发布ros消息
            return true;
        }
        case DATAINFO:
        {
            m_data_info_pub.publish(rcmData2ros(m_options.node_id, dataInfo));   //发布ros消息
            return true;
        }
        case ECHOEDINFO:
        {
            printf("\nEchoed range received.\n");
            m_echoed_range_pub.publish(rcmEcho2ros(echoedInfo));   //发布ros消息
            return true;
        }
    } // switch
...
void uavos::TimeDomainInterface::sendDataToNetworkCallback(const std_msgs::UInt8MultiArray::ConstPtr &msg){
    ...
    for(int i=0;i<msg->data.size();++i){
        byte_array[i] = msg->data.at(i);  // 将msg数据存入byte_array指针对应的内存中
    }
    rcmDataSend(RCRM_ANTENNAMODE_TXA_RXA, msg->data.size(), byte_array);  // 作者对函数进行了修改，注释掉一些内容？
