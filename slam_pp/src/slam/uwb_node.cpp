...
uavos::UWB_Mobile::UWB_Mobile(const int node_id,
    ...
    :UWB_Node(node_id),
      ...
      m_nh(nh)
{
    // subscribe uav state  (IMU Callback)
...
void uavos::UWB_Mobile::simpleInitializeEKF(){
    setPosition(10,10,0);   // set Tag's position with (10,10,0) arbitrarily
    setVelocity(0,0,0);
    setAccelerationBias(0,0,0);
    setSixStateCovariance(Eigen::MatrixXd::Identity(6,6)); // set Cov matrix as Identity
    ...
    for(std::map<int, boost::shared_ptr<uavos::UWB_Anchor> >::iterator iter=m_anchor_map.begin();
        ...
        ++iter)
        {
        int anchor_id = iter->second->getNodeId();  //get anchorId
...
void uavos::UWB_Mobile::getInnovation(){
    ...
    if(false==m_is_stablized){   // not stablized
...
bool uavos::UWB_Mobile::checkStablization(){
    ...
    for(int i=m_init_inspector_position_array.size()-1;i>=0;--i){
        ...
        if(ros::Time::now().toSec() - position.header.stamp.toSec() < 5){  //从当前位置对应的时间戳开始，查找时间戳差在5s以内的之前的所有位置，存入vaild_position_attay
    ...
    if(valid_position_array.size()<10){
        ...
        if(sigma_x<0.5 && sigma_y<0.5 && sigma_z<0.5){     // if variance is smaller than 0.5 , it can be seen as stablized
            return true;
        } else {
            return false;
        }
...
bool uavos::UWB_Mobile::kalmanFilter3DUpdate(const common_msgs::UWB_FullRangeInfo& range_info){
    ...
    // INPUTS:
    // X - the 6x1 a priori state vector - [x,xdot,y,ydot,z,zdot]'     6状态向量（先验）
    // P - the 6x6 a priori covariance matrix                          先验的协方差矩阵
    // T - the delta time between updates                              delta T
    // sigma_r - the estimated standard deviation of the new range measurement      测量值的标准差
    // r_meas - the actual range measurement                                        测量值
    // XYZ0 - the [x y z] location of the associated anchor node                    锚点位置 

    // OUTPUTS:
    // X - the new state vector estimate                               6状态向量（后验）
    // P - the new covariance matrix estimate                          后验的协方差矩阵
    // innovation - the difference between the range measurement and the estimated  测量值与估计值之差
    // range measurement (perhaps useful for outlier filtering.)       测量值可能是正确值也可能是离群值


    int anchor_id = range_info.responderId;
    readingCounterInc(anchor_id); // for statistics, to evaluate valid frequency

    // determine whether to perform filter using SNR  SNR为信噪比（这块不懂）
...
//    double T = range_info.header.stamp.toSec() - getRangeInfoToAnchor(anchor_id).header.stamp.toSec();
    ...
    double r_meas = double(range_info.precisionRangeMm) / 1000.0;   //转换成m为单位

    Eigen::Vector3d XYZ0 = Eigen::VectorXd::Zero(3);
    geometry_msgs::PointStamped anchor_position = m_anchor_map.find(anchor_id)->second->getPosition();  //得到锚点的位置
    ...
    uavos::Matrix6d M = F*P*F.transpose() + Q;    //先验的协方差矩阵

    // r_pred is the predicted range measure      //根据先验（预测值）计算估计的距离
    double r_pred = std::sqrt( std::pow(X(0) - XYZ0(0), 2) +
                               std::pow(X(2) - XYZ0(1), 2) +
                               std::pow(X(4) - XYZ0(2), 2) ) + 1e-6;  // 加号？？
    // H is the linearized measurement matrix    H为雅可比矩阵
    ...
    P = ( Eigen::MatrixXd::Identity(6,6) - K*H ) * M;    //后验的协方差矩阵
...
//    }
    ...
    // here we can use SVD for matrix square root, because P is guaranteed to be sysmetric,  // P是对称矩阵且可对角化
...
bool uavos::UWB_Mobile::ekfWithAcc(const common_msgs::UWB_FullRangeInfo &range_info){
    ...
    // T - the delta time between updates                                                    
    // tao_acc - variance of acceleratmetor's gaussian noise(IMU's measurment gaussian noise)
    // tao_bias - variance of acceleratmetor bias's noise
    // sigma_r - the estimated standard deviation of the new range measurement(UWB range measurement's noise)
    // r_meas - the actual range measurement
    // XYZ0 - the [x y z] location of the associated anchor node
    ...
    int anchor_id = range_info.responderId;
    readingCounterInc(anchor_id); // for statistics, to evaluate valid frequency(计数器)
...
//      }
    ...
    F.block<3,3>(6,6) = block_F;  //construct State Transition Matrix
    ...
    Q *= m_Q_scale;   // construct process noise covariance matrix 
    ...
    B.block<3,3>(6,6) = block_B;    // construct input gain matrix
    ...
    // K is the Kalman Gain
    double R = std::pow(sigma_r,2) * m_R_scale;  // UWB range measurement noise covariance matrix
    ...
    if(innovation<m_innovation_threshold){
        ...
        setRangeInfoToAnchor(range_info);  // save this time's range info
        m_last_range_time = range_info.header.stamp;

        successfulReadingCounterInc(anchor_id); // for statistics, to evaluate valid frequency(成功读取计数器)
...
bool uavos::UWB_Mobile::ukfWithAcc(const common_msgs::UWB_FullRangeInfo &range_info){
    ...
    // P - the 9x9 a priori covariance matrix
    // T - the delta time between updates
    // tao_acc - variance of acceleratmetor's gaussian noise. IMU measurement noise
    // tao_bias - variance of acceleratmetor bias's noise. IMU acceleration bias noise.
    // sigma_r - the estimated standard deviation of the new range measurement
    // r_meas - the actual range measurement
    // XYZ0 - the [x y z] location of the associated anchor node
  ...
  //    }
    ...
    // ========================== UKF ==============================
    // dimension
    int L = X.rows();  // total number of row
    double ukf_lambda = m_ukf_alpha*m_ukf_alpha * (L+m_ukf_kappa) - L;   // ukf_lambda即笔记中的κ , m_ukf_kappa即笔记中的λ
    double measurement_covariance = std::pow(sigma_r, 2) * m_R_scale;
    ...
    // which is proven to be diagonalizable.
    Eigen::MatrixXd P_prime = (L+ukf_lambda)*P;   
    // P_prime is a 9*9 matrix 且是个实对称阵，一定可以正交相似对角化，即 ∧ = trans(Q)*A*Q，倒过来即 A = Q*∧*trans(Q)
    Eigen::JacobiSVD<Eigen::MatrixXd> svd(P_prime, Eigen::ComputeFullV);
    Eigen::MatrixXd V = svd.matrixV();  // V matrix
    Eigen::VectorXd S = svd.singularValues();  //∑对角线元素 形成的向量
    Eigen::MatrixXd D = Eigen::MatrixXd::Zero(L,L); // U matrix
    for(int i=0;i<L;++i){
        D(i,i) = std::sqrt(S(i));
    }
    Eigen::MatrixXd P_prime_sqrt = V*D*V.transpose();  // sqrt(A) = Q*sqrt(∧)*trans(Q)
    ...
    for(int i=0;i<X_sigma_bar.cols();++i){
        X_sigma_bar.col(i) += B*u;   // χ_k|k-1
    }
    Eigen::VectorXd X_bar = X_sigma_bar * W_m;   // 状态 先验
    // calculate P_bar
    Eigen::MatrixXd P_bar = Q;
    for(int i=0;i<2*L+1;++i){
        P_bar += W_c(i) * (X_sigma_bar.col(i)-X_bar) * (X_sigma_bar.col(i)-X_bar).transpose();
