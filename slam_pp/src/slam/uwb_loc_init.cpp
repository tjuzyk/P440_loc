...
bool uavos::UWB_Loc_Init::initializeMobileByCeres(){
    ...
    if( solveOptimizationProblem() ){
        ...
        ROS_INFO("Initialization with Ceres OK. %f, %f, %f",
...
bool uavos::UWB_Loc_Init::getValidNeighborDatabse(){
    const int anchor_number = m_p_mobile->getAnchorNumber();  //get the total number of anchor
    ...
    while(valid_anchor_counter<anchor_number){
        ...
        // if the distance between tag and anchor is larger than 0.3,this anchor can be seen as a valid anchor
        for(int i=0;i<m_ndb.numNeighborEntries;++i){ 
            geometry_msgs::PointStamped anchor_position;
            std::cout<<"anchorId: "<<m_ndb.neighbors.at(i).nodeId<<", Range(mm): "<<m_ndb.neighbors.at(i).rangeMm<<std::endl;
