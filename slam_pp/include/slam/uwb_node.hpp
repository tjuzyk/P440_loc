...
public:
    std::map<int, std::vector<double> > m_range_map;   //std::map为模板类，需要关键字和存储对象这两个参数（<  >中的两个参数）。
...
public:
    ...
    inline void setRangeInfoToAnchor(const common_msgs::UWB_FullRangeInfo& range_info){
        m_ranges_to_anchors[range_info.responderId] = range_info;  // key——anchorId , key value——Range Info
    }
    ...
    void navigationStateCallback(const common_msgs::NavigationState::ConstPtr& msg){
        ...
        // threshold the acceleration to prevent filter divergence    做加速度限幅
